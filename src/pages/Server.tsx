  ...
  const refreshPlayers = useCallback(() => {
    setServerData((prev) =>
      prev.map((s) => {
        ...
        const newPlayers = Math.max(0, Math.min(100, s.players + delta));
  ...
  useEffect(() => {
    const updateMaintenance = () => {
      ...
      if (now < maintenanceStart) {
        setMaintenanceStatus('before');
        setMaintenanceCountdown(formatCountdown(maintenanceStart.getTime() - now.getTime()));
      } else if (now < maintenanceEnd) {
        setMaintenanceStatus('during');
        setMaintenanceCountdown(formatCountdown(maintenanceEnd.getTime() - now.getTime()));
      } else {
        setMaintenanceStatus('after');
        setMaintenanceCountdown('');
      }
    };
    updateMaintenance();
    const timer = setInterval(updateMaintenance, 1000);
    return () => clearInterval(timer);
  }, []);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-fy-green-dim/20 mb-10 sm:mb-16">
          {serverData.map((s) => {
            ...
            const playerPercent = Math.min(100, Math.max(0, (s.players / 100) * 100));
            return (
              <div key={s.name} className="bg-fy-panel p-4 sm:p-6 hud-corners hover:bg-fy-surface transition-colors">
                ...
                <div className="flex items-end gap-2 mb-1">
                  <span className="data-value text-2xl sm:text-3xl leading-none">{s.players}</span>
                  <span className="font-hud text-fy-steel text-xs sm:text-sm mb-1">/100</span>
                </div>
                ...
                <div className="space-y-2 mt-3 mb-4 sm:mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    <Server className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-fy-steel/40" />
                    <span className="font-mono text-[10px] sm:text-[11px] text-fy-steel/60 tracking-wider">{s.ip}:{s.port}</span>
                  </div>
                </div>
          ...
          <div className="panel p-0">
            ...
            <div className="px-6 py-5">
              <div className="border-l-2 border-fy-amber/60 pl-4 py-1">
                <p className="font-hud font-semibold text-white text-sm tracking-wider">服务器例行维护</p>
                <p className="font-mono text-fy-amber/70 text-xs mt-1">2026-06-03 02:00 — 04:00</p>
                <p className="text-fy-steel/40 text-xs mt-2">预计维护2小时，停机12~24小时补1天预留位时长</p>
                {/* 维护倒计时 */}
                {maintenanceCountdown && (
                  <div className="mt-3 flex items-center gap-2">
                    <Clock className="w-3.5 h-3.5 text-fy-amber/60" />
                    <span className="font-mono text-fy-amber text-xs tracking-wider">
                      {maintenanceStatus === 'before' ? `距离维护开始: ${maintenanceCountdown}` : `距离维护结束: ${maintenanceCountdown}`}
                    </span>
                  </div>
                )}
          ...
          <div className="panel p-0">
            <div className="flex items-center justify-between px-6 py-4 border-b border-fy-green-dim/20">
              ...
              <div className="flex gap-1">
                {([
                  { key: 'all' as BanFilter, label: '全部' },
                  { key: 'permanent' as BanFilter, label: '永久' },
                  { key: 'temp' as BanFilter, label: '临时' },
                ]).map((f) => (
                  <button
                    key={f.key}
                    onClick={() => setBanFilter(f.key)}
                    className={`font-mono text-[10px] tracking-wider px-2 py-0.5 border transition-colors ${
                      banFilter === f.key
                        ? 'border-fy-amber/50 text-fy-amber bg-fy-amber/10'
                        : 'border-fy-green-dim/20 text-fy-steel/40 hover:text-fy-steel/60'
                    }`}
                  >
                    {f.label}
                  </button>
            ...
            <div className="px-0">
              {filteredBans.length === 0 && (
                <div className="px-6 py-4 text-fy-steel/30 font-mono text-xs text-center">无匹配记录</div>
              )}
              {filteredBans.map((b, i) => (
                <div key={b.name + b.type} className={`flex items-center justify-between px-6 py-3 ${i < filteredBans.length - 1 ? 'divider' : ''}`}>
                  <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-fy-green-dim/10">
          {funStats.map((s) => (
            <div key={s.label} className="bg-fy-panel p-4 sm:p-6 hover:bg-fy-surface transition-colors">
              <s.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-fy-amber/50 mb-2 sm:mb-3" />
              <div className="font-hud font-bold text-white text-lg sm:text-xl leading-none mb-1">{s.value}</div>
              <div className="data-label text-[10px] sm:text-xs">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
