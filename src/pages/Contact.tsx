              ...
              {infoItems.map((item) => (
                <div key={item.label}>
                  <div className="flex items-center gap-2 mb-1">
                    <item.icon className="w-3.5 h-3.5 text-fy-amber" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                <div>
                  ...
                  <input type="text" value={nickname} onChange={(e) => setNickname(e.target.value)} className="w-full bg-transparent border-b border-fy-green-dim/30 py-2.5 sm:py-3 text-white font-mono text-sm outline-none focus:border-fy-amber transition-colors placeholder:text-fy-steel/30" placeholder="你的称呼" />
                </div>
                <div>
                  <label className="data-label block mb-2">联系方式 *</label>
                  <input
                    type="text"
                    value={contact}
                    onChange={(e) => { setContact(e.target.value); if (touched.contact) setErrors(validate()); }}
                    onBlur={() => handleBlur('contact')}
                    className={`w-full bg-transparent border-b py-2.5 sm:py-3 text-white font-mono text-sm outline-none transition-colors placeholder:text-fy-steel/30 ${touched.contact && errors.contact ? 'border-fy-red focus:border-fy-red' : 'border-fy-green-dim/30 focus:border-fy-amber'}`}
                    placeholder="QQ或邮箱"
                  />
              ...
              <div>
                <label className="data-label block mb-2">类型</label>
                <select className="w-full bg-transparent border-b border-fy-green-dim/30 py-2.5 sm:py-3 text-white font-mono text-sm outline-none focus:border-fy-amber transition-colors appearance-none cursor-pointer">
                  <option value="bug" className="bg-fy-panel">问题反馈</option>
                  <option value="suggest" className="bg-fy-panel">功能建议</option>
                  <option value="other" className="bg-fy-panel">其他</option>
                </select>
              </div>
              <div>
                <label className="data-label block mb-2">内容 *</label>
                <textarea
                  rows={4}
                  value={content}
                  onChange={(e) => { setContent(e.target.value); if (touched.content) setErrors(validate()); }}
                  onBlur={() => handleBlur('content')}
                  className={`w-full bg-transparent border-b py-2.5 sm:py-3 text-white font-mono text-sm outline-none transition-colors resize-none placeholder:text-fy-steel/30 ${touched.content && errors.content ? 'border-fy-red focus:border-fy-red' : 'border-fy-green-dim/30 focus:border-fy-amber'}`}
